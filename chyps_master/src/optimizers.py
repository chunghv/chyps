import torch
from chyps import Sps
from chyps import Chyps
...
def get_optimizer(opt_dict, params, train_loader, exp_dict):
    ...
    if opt_name == 'sps':
        ...
        opt = Chyps(params,
                    n_batches_per_epoch=n_batches_per_epoch,
                    init_step_size=opt_dict.get('init_step_size', 1e-3),
                    gamma=opt_dict.get('gamma', 0.9),
                    epsilon=opt_dict.get('epsilon', 1e-8),
                    beta=opt_dict.get('beta', 10.0),
                    tau=opt_dict.get('tau', 2.0),
                    option=opt_dict.get('option', 'II'),
                    centralize_grad=opt_dict.get("centralize_grad", False))
    # others        
    elif opt_name == "adam":
